...
| `game.klondike` | Playable Klondike | Partial | The native new-deal controls expose draw-one, draw-three, Standard, and Vegas scoring; focused controller coverage preserves all four exact choices through atomic save/reopen. Malformed option requests are rejected before deal-number reservation and preserve the current game, save, counter, and pending request. An installed keyboard stock draw, undo/redo, and save/reopen were accepted at exact SHA f6b0cb7e55d296bdf77714efc48a1775b858c041. The Vegas UI workflow still needs exact-package Wayland acceptance. Both scoring modes are untimed with unlimited stock passes; timed play, limited redeals, left-handed layout, drag/drop, a complete-deal win presentation, and broader platform evidence remain open. |
